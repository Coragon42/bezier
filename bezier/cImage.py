...
class AbstractImage(object):
    ...
    def __init__(self,fname=None,data=[],imobj=None,height=0,width=0):
        ...
        if fname:
            ...
            self.im = imobj.copy() # definition of im

        if pilAvailable:
            self.width,self.height = self.im.size
        else:
            self.width = self.im.width() # AttributeError: EmptyImage has no attribute 'im'
...
class EmptyImage(AbstractImage):
    im = None # I think this fixed it?
    def __init__(self,cols,rows):
        self.im=tkinter.PhotoImage(height=rows,width=cols) # I think this fixed it?
        super(EmptyImage, self).__init__(height = rows, width = cols) # AttributeError
...
if __name__ == '__main__':
    ...
    oImage = FileImage('earth-rise.gif')
...
#             x = map(lambda x: 255-x, v)
    ...
    myImage.draw(win)
    print(win.getMouse())
    myImage.save('earth-rise-inverted.gif')
